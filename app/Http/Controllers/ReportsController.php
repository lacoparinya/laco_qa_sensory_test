...
class ReportsController extends Controller
{
    ...
    public function rangereport(){
        return  view('reports.rangereport');
    }

    public function rangereportAction(Request $request)
    {

        $requestData = $request->all();

        $masterData = SensoryMaster::whereBetween('test_date', [$requestData['from_date'], $requestData['to_date']])->get();
        ...
        $summaryData = DB::table('sensory_test_ms')
            ...
            ->select(
                ...
                DB::raw('count(sensory_test_ms.tester_name) as number_tester')
            )->whereBetween('sensory_masters.test_date', [$requestData['from_date'], $requestData['to_date']]
            )->groupBy(
                'sensory_masters.test_date',
                'sensory_details.code',
                'qa_sample_sensories.product_name',
        ...
        $detailData = DB::table('sensory_test_ms')
            ...
            ->select(
                ...
                'sensory_test_ds.texture',
                'sensory_test_ds.taste'
            )->whereBetween('sensory_masters.test_date', [$requestData['from_date'], $requestData['to_date']])->get();

        $detailSummary = array();
        foreach ($detailData as $key => $value) {
            if (isset($detailSummary[$value->test_date][$value->code])) {
                $detailSummary[$value->test_date][$value->code]['count'] += 1;
                if ($value->color == 1 || $value->color == 5) {
                    $detailSummary[$value->test_date][$value->code]['color']['fail'] += 1;

                    $detailSummary[$value->test_date][$value->code]['all']['fail'] += 1;
                } else {
                    $detailSummary[$value->test_date][$value->code]['color']['pass'] += 1;

                    $detailSummary[$value->test_date][$value->code]['all']['pass'] += 1;
                }
                if ($value->odor == 1 || $value->odor == 5) {
                    $detailSummary[$value->test_date][$value->code]['odor']['fail'] += 1;

                    $detailSummary[$value->test_date][$value->code]['all']['fail'] += 1;
                } else {
                    $detailSummary[$value->test_date][$value->code]['odor']['pass'] += 1;

                    $detailSummary[$value->test_date][$value->code]['all']['pass'] += 1;
                }
                if ($value->texture == 1 || $value->texture == 5) {
                    $detailSummary[$value->test_date][$value->code]['texture']['fail'] += 1;

                    $detailSummary[$value->test_date][$value->code]['all']['fail'] += 1;
                } else {
                    $detailSummary[$value->test_date][$value->code]['texture']['pass'] += 1;

                    $detailSummary[$value->test_date][$value->code]['all']['pass'] += 1;
                }
                if ($value->taste == 1 || $value->taste == 5) {
                    $detailSummary[$value->test_date][$value->code]['taste']['fail'] += 1;

                    $detailSummary[$value->test_date][$value->code]['all']['fail'] += 1;
                } else {
                    $detailSummary[$value->test_date][$value->code]['taste']['pass'] += 1;
                ...
                $detailSummary[$value->test_date][$value->code]['count'] = 1;
                if ($value->color == 1 || $value->color == 5) {
                    $detailSummary[$value->test_date][$value->code]['color']['pass'] = 0;
                    $detailSummary[$value->test_date][$value->code]['color']['fail'] = 1;

                    $detailSummary[$value->test_date][$value->code]['all']['pass'] = 0;
                    $detailSummary[$value->test_date][$value->code]['all']['fail'] = 1;
                } else {
                    $detailSummary[$value->test_date][$value->code]['color']['pass'] = 1;
                    $detailSummary[$value->test_date][$value->code]['color']['fail'] = 0;

                    $detailSummary[$value->test_date][$value->code]['all']['pass'] = 1;
                    $detailSummary[$value->test_date][$value->code]['all']['fail'] = 0;
                }
                if ($value->odor == 1 || $value->odor == 5) {
                    $detailSummary[$value->test_date][$value->code]['odor']['pass'] = 0;
                    $detailSummary[$value->test_date][$value->code]['odor']['fail'] = 1;

                    $detailSummary[$value->test_date][$value->code]['all']['fail'] += 1;
                } else {
                    $detailSummary[$value->test_date][$value->code]['odor']['pass'] = 1;
                    $detailSummary[$value->test_date][$value->code]['odor']['fail'] = 0;

                    $detailSummary[$value->test_date][$value->code]['all']['pass'] += 1;
                }
                if ($value->texture == 1 || $value->texture == 5) {
                    $detailSummary[$value->test_date][$value->code]['texture']['pass'] = 0;
                    $detailSummary[$value->test_date][$value->code]['texture']['fail'] = 1;

                    $detailSummary[$value->test_date][$value->code]['all']['fail'] += 1;
                } else {
                    $detailSummary[$value->test_date][$value->code]['texture']['pass'] = 1;
                    $detailSummary[$value->test_date][$value->code]['texture']['fail'] = 0;

                    $detailSummary[$value->test_date][$value->code]['all']['pass'] += 1;
                }
                if ($value->taste == 1 || $value->taste == 5) {
                    $detailSummary[$value->test_date][$value->code]['taste']['pass'] = 0;
                    $detailSummary[$value->test_date][$value->code]['taste']['fail'] = 1;

                    $detailSummary[$value->test_date][$value->code]['all']['fail'] += 1;
                } else {
                    $detailSummary[$value->test_date][$value->code]['taste']['pass'] = 1;
                    $detailSummary[$value->test_date][$value->code]['taste']['fail'] = 0;

                    $detailSummary[$value->test_date][$value->code]['all']['pass'] += 1;
                }
            }

            $percent = ($detailSummary[$value->test_date][$value->code]['taste']['fail'] / $detailSummary[$value->test_date][$value->code]['count']) * 100;

            if ($percent < 20) {
                $detailSummary[$value->test_date][$value->code]['result'] = 'Pass';
            } else {
                $detailSummary[$value->test_date][$value->code]['result'] = 'Fail';
            }
        }

        $filename = "qa_sensory_report_range_" . date('ymdHi');

        Excel::create($filename, function ($excel) use ($masterData, $summaryData, $detailSummary) {
            $excel->sheet('clients', function ($sheet) use ($masterData, $summaryData, $detailSummary) {
                $sheet->setOrientation('landscape')->loadView('reports.xlsrangereport')->with('masterData', $masterData)->with('summaryData', $summaryData)->with('detailSummary', $detailSummary);
            });
        })->export('xlsx');
    }
}
